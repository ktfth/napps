...
describe('Nap Subject', () => {
  it('should be the instance', () => {
    let subject = new nap.Subject();
    assert.ok(subject instanceof nap.Subject);
  });
  ...
  it('should have set state', () => {
    let subject = new nap.Subject();
    assert.ok(subject.setState(1) instanceof nap.Subject);
    assert.equal(subject.getState(), 1);
  });
});

describe('Nap Observer', () => {
  it('should be the instance', () => {
    let observer = new nap.Observer();
    assert.ok(observer instanceof nap.Observer);
  });

  it('should be the instance of Subject', () => {
    let subject = new nap.Subject();
    let observer = new nap.Observer();
    assert.ok(subject.attach(observer) instanceof nap.Subject);
  });

  it('should have update', () => {
    let observer = new nap.Observer();
    assert.ok(observer.update((a, b) => a + b, 1, 2) instanceof nap.Observer);
  });
});
